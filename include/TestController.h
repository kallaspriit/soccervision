...
public:
	...
	public:
		FetchBallBehindState(TestController* ai) : State(ai), hadBall(false), lastTargetAngle(0.0f), lostBallTime(0.0), timeSinceLostBall(0.0), lostBallVelocity(0.0f), smallestBallDistance(-1.0f), targetMode(TargetMode::UNDECIDED) {}
		void onEnter(Robot* robot);
		void step(float dt, Vision::Results* visionResults, Robot* robot, float totalDuration, float stateDuration);
