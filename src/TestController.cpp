...
void TestController::AccelerateState::step(float dt, Vision::Results* visionResults, Robot* robot, float totalDuration, float stateDuration) {
	...
	float ballDistance = ball->getDribblerDistance();
	float brakeDistance = Math::getAccelerationDistance(currentSpeed, 0.0f, acceleration) * 2.0f; // 2.0
	float forwardSpeed;

	if (ballDistance < brakeDistance) {
		float brakeAcceleration = Math::getAcceleration(currentSpeed, 0.0f, brakeDistance) * 2.0f;
