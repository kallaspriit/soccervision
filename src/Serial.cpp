...
*
* v1.02: Fixed a bug in BufferedSerial: Using the default constructor
...
{
public:
	SerialImpl() : io(), port(io), backgroundThread(), open(false),
		error(false) {}
	...
	char readBuffer[Serial::readBufferSize]; ///< data being read

	/// Read complete callback
	boost::function<void(const char*, size_t)> callback;
};

Serial::Serial() : pimpl(new SerialImpl)
{
	...
	asio::serial_port_base::stop_bits opt_stop)
{
	...
	pimpl->io.post(boost::bind(&Serial::doRead, this));
...
void Serial::close()
{
	...
	pimpl->open = false;
	pimpl->io.post(boost::bind(&Serial::doClose, this));
...
void Serial::write(const char *data, size_t size)
{
	...
	pimpl->io.post(boost::bind(&Serial::doWrite, this));
}
...
void Serial::doRead()
{
	pimpl->port.async_read_some(asio::buffer(pimpl->readBuffer, readBufferSize),
		boost::bind(&Serial::readEnd,
		this,
		asio::placeholders::error,
		asio::placeholders::bytes_transferred));
}

void Serial::readEnd(const boost::system::error_code& error,
...
void Serial::writeEnd(const boost::system::error_code& error)
{
	if (!error)
	{
		lock_guard<mutex> l(pimpl->writeQueueMutex);
...
void Serial::setReadCallback(const boost::function<void(const char*, size_t)>& callback)
{
...
{
public:
	SerialImpl() : backgroundThread(), open(false), error(false) {}
	...
	asio::serial_port_base::stop_bits opt_stop)
{
	...
	thread t(bind(&Serial::doRead, this));
	pimpl->backgroundThread.swap(t);
}
...
void Serial::setReadCallback(const
...
void CallbackSerial::setCallback(const
	boost::function<void(const char*, size_t)>& callback)
{
